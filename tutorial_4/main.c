...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        {
            /* We got no message, thie means the timeout expired */
            if (data.playing)
            {
                gint64 current = -1;
                ...
                g_message("Position %" GST_TIME_FORMAT " / %" GST_TIME_FORMAT "\r", GST_TIME_ARGS(current), GST_TIME_ARGS(data.duration));
        ...
        if (GST_MESSAGE_SRC(msg) == GST_OBJECT(data->playbin))
        {
            ...
            if (data->playing)
            {
                ...
                if (gst_element_query(data->playbin, query))
                {
                    ...
                    {
                        g_message("Seeking is ENABLED from %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT "\n",
                                  GST_TIME_ARGS(start), GST_TIME_ARGS(end));
                    }
                    else
                    {
                        g_message("Seeking is DISABLED for this stream.\n");
                    }
                }
                else
                {
