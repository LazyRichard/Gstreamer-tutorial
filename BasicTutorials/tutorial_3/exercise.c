...
int main(int argc, char *argv[])
{
    ...
    if (!data.pipeline || !data.source || !data.videoConvert || !data.audioConvert ||
        !data.audioResample || !data.videoSink || !data.audioSink)
    {
    ...
    gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.videoConvert, data.audioConvert,
                     data.audioResample, data.videoSink, data.audioSink, NULL);
