    ...
    if (msg != NULL) {
      ...
      case GST_MESSAGE_STATE_CHANGED:
        ...
        if (GST_MESSAGE_SRC(msg) == GST_OBJECT(pipeline)) {
          ...
          g_message("Pipeline state change from %s to %s:", gst_element_state_get_name(old_state),
                    gst_element_state_get_name(new_state));

          /* Create graph per state change */
          GString *base_fname = g_string_new("");
          g_string_printf(base_fname, "pipeline-%s-%s", gst_element_state_get_name(old_state),
                          gst_element_state_get_name(new_state));
